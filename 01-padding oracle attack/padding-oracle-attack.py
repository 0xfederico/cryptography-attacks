...
# C1 manipulation -> HC1
# P manipulated = current padding

# XOR -> if equal 0 ; if differs 1
...
if __name__ == '__main__':
	msg = lorem.text().encode('utf8')
	cracked_ct = attack(encrypt(msg))
	assert len(cracked_ct) == len(msg), f"{len(cracked_ct)} != {len(msg)}"
	assert cracked_ct == msg, f"{cracked_ct} != {msg}"
	# print(cracked_ct.decode('utf8'))
